...
import { IconCopy, IconTrash, IconDownload, IconCheck } from '../components/Icons'
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 animate-fade-in">
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
          <div className="bg-white border-2 border-ink-200 p-8 max-w-sm w-full mx-4 animate-slide-up">
            <h3 className="font-display font-bold text-lg text-ink-950 mb-3">确认清空</h3>
            <p className="text-ink-600 text-sm mb-6">此操作将永久删除所有历史记录，且无法恢复。</p>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="font-display font-bold text-3xl text-ink-950">历史记录</h1>
          <p className="text-ink-500 mt-2">共 {records.length} 条转换记录</p>
        </div>
      ...
      {records.length === 0 ? (
        <div className="text-center py-20 border-2 border-ink-200 bg-white">
          <p className="text-ink-400 font-display text-lg">暂无历史记录</p>
          <p className="text-ink-300 text-sm mt-2">开始转换后，记录将自动保存在这里</p>
        </div>
      ) : (
        <>
          {records.length > 1 && (
            <div className="mb-4">
              <label className="flex items-center gap-2 cursor-pointer text-sm text-ink-600 hover:text-ink-950 transition-colors">
                <input
                  type="checkbox"
                  checked={selected.size === records.length}
                  onChange={toggleSelectAll}
                  className="w-4 h-4 cursor-pointer accent-ink-950"
                />
                全选
              </label>
            </div>
          ...
          <div className="space-y-0">
            {records.map((r, i) => (
              <div
                key={r.id}
                className={`border-2 border-ink-200 -mt-[2px] first:mt-0 p-5 bg-white transition-all duration-200 hover:border-ink-400 animate-slide-up opacity-0`}
              ...
              >
                <div className="flex items-start gap-4">
                  <input
                    ...
                    className="mt-1 w-4 h-4 cursor-pointer accent-ink-950 flex-shrink-0"
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span className="font-mono text-xs bg-ink-100 text-ink-700 px-2 py-0.5">{getCitationTypeName(r.citation.type)}</span>
                      <span className="font-mono text-xs bg-vermilion-50 text-vermilion-700 px-2 py-0.5">{getFormatName(r.targetFormat)}</span>
                      <span className="text-xs text-ink-400 ml-auto flex-shrink-0">{formatTimestamp(r.timestamp)}</span>
                    </div>
                    <p className="text-sm text-ink-950 leading-relaxed font-body break-all">{r.result}</p>
                  </div>
