...
exports.parseCitationText = parseCitationText;
exports.parseEnglishCitation = parseEnglishCitation;
exports.parseEnglishAPA = parseEnglishAPA;
const utils_1 = require("./utils");
const publisher_places_1 = require("./publisher-places");
function fillPublishPlace(c) {
    if (c.type === 'book' && !c.publishPlace && c.publisher) {
        const place = (0, publisher_places_1.lookupPlace)(c.publisher);
        if (place)
            return { ...c, publishPlace: place };
    }
    return c;
}
function parseCitationText(text) {
    ...
    const language = (0, utils_1.detectLanguage)(normalizedText);
    const singleLine = normalizedText.replace(/\s+/g, ' ');
    const type = detectType(singleLine, language);
    let result = { id, language, type, rawText: singleLine };
    // 古籍：自动推断子类型
    if (type === 'ancient') {
...
function parseDoubanBook(text) {
    ...
    if (yearM)
        result.publishYear = yearM[1];
    // 页数存入 notes 做提示
    if (fields['页数'])
        result.notes = `共${fields['页数']}页`;
    return result;
}
// ─── GB/T 7714 格式解析 ─────────────────────────────────────────────
function parseGBT7714Style(text) {
    ...
    const result = { type, title, authors };
    if (type === 'thesis') {
        // 学位论文：提取 thesisType
        ...
        if (instMatch) {
            ...
            if (schoolMatch)
                result.institution = schoolMatch[1].trim();
        }
        const yearMatch = /(\d{4})/.exec(afterTag);
        if (yearMatch)
            result.publishYear = yearMatch[1];
    }
        ...
        if (journalMatch) {
            ...
            if (journalMatch[3])
                result.volumeNumber = journalMatch[3];
            if (journalMatch[4])
                result.issue = journalMatch[4];
            if (journalMatch[5])
                result.pages = journalMatch[5];
        }
        else {
            const nameMatch = /^([^,，]+)/.exec(afterTag);
            if (nameMatch)
                result.journalName = nameMatch[1].trim();
            const yearMatch = /(\d{4})/.exec(afterTag);
            if (yearMatch)
                result.publishYear = yearMatch[1];
...
function detectType(text, lang) {
    if (text.includes('转引自'))
        return 'transferred';
    if (/https?:\/\//.test(text))
        return 'electronic';
    if (lang === 'zh') {
        // 典津古籍标记 → 直接识别为古籍刻本
        if (text.includes('\u3010\u5178\u6D25\u53E4\u7C4D\u3011'))
            return 'ancient';
        // 古籍版本关键词（优先于其他检测）
        if (/刻本|抄本|寫本|写本|影印本|活字本|重寫本|稿本/.test(text))
            return 'ancient';
        if (/《[^》]+》.*?\d{4}年.*?第\d+期/.test(text))
            return 'journal';
        if (/报.*?\d{4}年\d{1,2}月\d{1,2}日.*?第\d+版/.test(text))
            return 'newspaper';
        if (/学位论文|硕士论文|博士论文/.test(text))
            return 'thesis';
        if (/会议|论文集/.test(text))
            return 'conference';
        if (/档案|卷宗|编号/.test(text))
            return 'archive';
    }
    else {
        // APA/英文期刊特征
        if (/\(\d{4}\)\.\s/.test(text) || /jstor\.org/.test(text))
            return 'journal';
        if (/Vol\.|No\.|pp\./i.test(text))
            return 'journal';
        if (/dissertation|thesis/i.test(text))
            return 'thesis';
...
function inferAncientSubType(text) {
    if (/刻本|活字本/.test(text))
        return 'blockprint';
    if (/抄本|寫本|写本|稿本/.test(text))
        return 'blockprint';
    if (/影印本/.test(text))
        return 'reprint';
    if (/点校|标点本|整理本/.test(text))
        return 'punctuated';
    return 'blockprint';
}
// ─── 中文解析 ────────────────────────────────────────────────────
function parseChineseCitation(text, type) {
    const result = {};
    ...
    if (titleMatch) {
        ...
        if (secondTitleMatch) {
            if (type === 'journal')
                result.journalName = secondTitleMatch[1];
            else
                result.bookTitle = secondTitleMatch[1];
        }
    }
    // 学位论文特殊解析
    if (type === 'thesis') {
    ...
    if (type === 'conference') {
        ...
        if (placeMatch)
            result.publishPlace = placeMatch[1].trim();
    }
...
// ─── 英文文献统一入口 ────────────────────────────────────────────
function parseEnglishCitation(text) {
...
function parseMLA(text) {
    ...
    if (volMatch)
        result.volumeNumber = volMatch[1];
    // no. Y
    const noMatch = /no\.?\s*(\d+)/i.exec(rest);
    if (noMatch)
        result.issue = noMatch[1];
    // 年份
    ...
    if (accessMatch)
        result.accessDate = accessMatch[1].trim();
    result.type = 'journal';
    return result;
}
function parseMLAAuthors(authorsPart) {
    if (!authorsPart.trim())
        return [];
    ...
    return segments.map(raw => {
        // 首字母大写
        const parts = raw.split(/,\s*/).map(p => p.trim().split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' '));
...
function parseEnglishAPA(text) {
    ...
    if (apaYearMatch) {
        ...
        if (yearMatch)
            result.publishYear = yearMatch[1] || yearMatch[2];
...
function parseAPAAuthors(authorsPart) {
    if (!authorsPart.trim())
        return [];
    // 分割 & 和 ,
