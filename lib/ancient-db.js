...
const { lookupPlace } = require('./publisher-places');
...
function loadDatabase() {
  ...
  const dbPath = path.join(__dirname, 'ancient-books-all.json');
...
function generateLSYJCitation(record, volumeNum = '', page = '') {
  ...
  // 出版信息：出版地：出版社，出版年
  if (record.publisher) {
    const place = record.publishPlace || lookupPlace(record.publisher);
    let pubPart = place ? `${place}：${record.publisher}` : record.publisher;
