...
exports.processBookTitleMarks = processBookTitleMarks;
exports.formatLSYJ = formatLSYJ;
 ...
 * 示例：《读〈石钟山记〉有感》
 */
function processBookTitleMarks(text) {
    if (!text)
        return text;
...
function authorStr(c) {
    if (!c.authors || c.authors.length === 0)
        return '';
    if (c.language === 'en') {
        const names = c.authors.map(a => (0, author_utils_js_1.formatAuthorName)(a, 'suffix', ['author']));
        const sep = c.authors.length <= 2 ? ' and ' : ', ';
        return (0, author_utils_js_1.joinAuthorNames)(names, { separator: sep });
    }
 ...
 * "42" → "第42册", "第88册" → "第88册"
 */
function formatSeriesVolume(vol) {
    if (!vol)
        return '';
    if (vol.includes('册'))
        return vol;
 ...
 */
function convertToChineseParens(text) {
    // 按 URL 分割：奇数段是 URL（保留原样），偶数段做替换
    const parts = text.split(/(https?:\/\/[^\s，。、；：]+)/);
    return parts.map((part, i) => {
        if (i % 2 === 1)
            return part; // URL 段，不转换
...
function formatLSYJRaw(citation) {
    const c = citation;
    ...
    switch (c.type) {
        case 'book': {
            if (isEn) {
                ...
                if (c.publishYear)
                    parts.push(c.publishYear);
                if (c.pages)
                    parts.push(pageStr(c.pages, 'en'));
        ...
        case 'chapter': {
            ...
            if (c.bookTitle)
                parts.push(processBookTitleMarks(`《${c.bookTitle}》`));
            if (c.volume)
                parts.push(c.volume);
            if (c.publishPlace && c.publisher)
                parts.push(`${c.publishPlace}：${c.publisher}`);
        ...
        case 'journal': {
            if (isEn) {
                ...
                if (c.journalName)
                    parts.push(`*${c.journalName}*`);
                if (c.volumeNumber)
                    parts.push(`Vol. ${c.volumeNumber}`);
                if (c.issue)
                    parts.push(`No. ${c.issue}`);
                if (c.publishYear)
                    parts.push(c.publishYear);
                if (c.pages)
                    parts.push(pageStr(c.pages, 'en'));
            ...
            if (c.volumeNumber && c.issue) {
                ...
                if (c.publishYear)
                    result += `${c.publishYear}年`;
                if (c.issue)
                    result += `第${removeLeadingZero(c.issue)}期`;
            }
        ...
        case 'newspaper': {
            ...
            result += processBookTitleMarks(`《${c.title}》`);
            if (c.newspaperName)
                result += `，` + processBookTitleMarks(`《${c.newspaperName}》`);
            if (c.publishDate)
                result += c.publishDate;
            if (c.pageSection)
                result += `，第${c.pageSection}版`;
            return result + '。';
        }
        case 'conference': {
            if (isEn) {
        ...
        case 'diary': {
            ...
            if (c.publishDate)
                result += `，${c.publishDate}`;
            if (c.translators && c.translators.length > 0)
                result += `，${translatorStr(c)}`;
            if (c.publishPlace && c.publisher)
                result += `，${c.publishPlace}：${c.publisher}`;
            if (c.publishYear)
                result += `，${c.publishYear}年`;
            if (c.pages)
                result += `，${pageStr(c.pages, 'zh')}`;
        ...
        case 'electronic': {
            ...
            result += processBookTitleMarks(`《${c.title}》`);
            if (c.journalName)
                result += `，` + processBookTitleMarks(`《${c.journalName}》`);
            if (c.issue)
                result += `${c.publishYear}年第${removeLeadingZero(c.issue)}期`;
            if (c.url)
                result += `，${c.url}`;
            if (c.accessDate)
                result += `，访问时间：${c.accessDate}`;
            return result + '。';
        }
        case 'transferred': {
            const auth = authorStr(c);
            ...
            if (auth)
                result = auth + '：';
            result += processBookTitleMarks(`《${c.title}》`);
            if (c.originalCitation)
                result += `，${c.originalCitation}`;
            if (c.transferredFrom)
                result += `，转引自${c.transferredFrom}`;
            if (c.pages)
                result += `，${pageStr(c.pages, 'zh')}`;
            return result + '。';
        }
...
function formatAncientBlockprint(c) {
    const parts = [];
    ...
    parts.push(processBookTitleMarks(`《${c.title}》`));
    if (c.volume)
        parts.push(formatVolume(c.volume));
    if (c.section)
        parts.push(processBookTitleMarks(`《${c.section}》`));
    if (c.ancientEdition)
        parts.push(c.ancientEdition);
    if (c.archiveLocation)
        parts.push(c.archiveLocation);
    if (c.pages) {
...
function formatAncientPunctuated(c) {
    const parts = [];
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
    if (dyn || auth)
...
function formatAncientReprint(c) {
    const parts = [];
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
    if (dyn || auth)
    ...
    if (c.bookletVolume)
        parts.push(formatBooklet(c.bookletVolume));
    if (c.pages) {
        // 影印本：第461页下栏
...
function formatAncientExtract(c) {
    const parts = [];
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
    if (dyn || auth)
    ...
    if (c.seriesVolume)
        parts.push(formatSeriesVolume(c.seriesVolume));
    if (c.pages)
        parts.push(pageStr(c.pages, 'zh'));
    return joinAncientParts(parts) + '。';
}
...
function formatAncientGazetteer(c) {
    const parts = [];
    // 地方志：修纂年代冠题名前，作者一般不标
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
...
function formatAncientClassic(c) {
    const parts = [];
    // 典籍一般不标作者
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
...
function formatAncientChronicle(c) {
    const parts = [];
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
    if (dyn || auth)
...
// ─── 向后兼容：未选子类型时用旧逻辑 ─────────────
function formatAncientLegacy(c) {
    const dyn = dynastyPrefix(c);
    const auth = authorStr(c);
    let result = '';
    if (dyn || auth)
    ...
    result += processBookTitleMarks(`《${c.title}》`);
    if (c.volume)
        result += formatVolume(c.volume);
    if (c.section)
        result += processBookTitleMarks(`《${c.section}》`);
    if (c.ancientEdition)
        result += `，${c.ancientEdition}`;
    if (c.seriesName)
        result += `，` + processBookTitleMarks(`《${c.seriesName}》`);
    if (c.publishPlace && c.publisher)
        result += `，${c.publishPlace}：${c.publisher}`;
    if (c.publishYear)
        result += `，${c.publishYear}年`;
    if (c.seriesVolume)
        result += `，${formatSeriesVolume(c.seriesVolume)}`;
    if (c.pages) {
...
// - 否则前面加"，"
function joinAncientParts(parts) {
    if (parts.length === 0)
        return '';
    let result = parts[0];
    for (let i = 1; i < parts.length; i++) {
        ...
        // 卷次后直接拼接（卷X + 篇名/下一个元素）
        if (next.startsWith('卷')) {
            result += next;
            continue;
        }
        // 卷次后的篇名直接拼接（卷X + 《篇名》）
        if (prev.startsWith('卷') && next.startsWith('《')) {
            result += next;
            continue;
        }
        // 其他情况加逗号
 ...
 * 不同角色分开：王安点校、谢安整理
 */
function punctuatorStr(punctuators) {
    if (!punctuators || punctuators.length === 0)
        return '';
