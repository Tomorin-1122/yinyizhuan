...
exports.PUBLISHER_PLACES = void 0;
 ...
 * 返回 undefined 表示无匹配
 */
function lookupPlace(publisher) {
    if (!publisher || !publisher.trim())
        return undefined;
    const p = publisher.trim();
    // 1. 精确匹配
    if (exports.PUBLISHER_PLACES[p])
        return exports.PUBLISHER_PLACES[p];
    // 2. 包含匹配（去掉空格和标点后再比，避免"出 版 社"这种干扰）
    const normalize = (s) => s.replace(/[\s·、，,]/g, '');
    const np = normalize(p);
    for (const [key, val] of Object.entries(exports.PUBLISHER_PLACES)) {
        if (normalize(key) === np)
            return val;
    }
    ...
    for (const [key, val] of Object.entries(exports.PUBLISHER_PLACES)) {
        if (key.length <= 6 && p.includes(key))
            return val;
        if (key.length <= 6 && key.includes(p))
            return val;
    }
    return undefined;
}
